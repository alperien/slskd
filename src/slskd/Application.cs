...
#pragma warning restore S1215 // "GC.Collect" should not be called
        ...
        private async void Client_LoggedIn(object sender, EventArgs e)
        {
            try
            {
                ...
                try
                {
                    ...
                    if (resumableDownloads.Any())
                    {
                        ...
                        // remove ALL old errored records so auto-replace doesn't attempt to replace them separately,
                        // including transfers that couldn't be re-enqueued (user offline, error, etc.)
                        Transfers.Downloads.Remove(t => ActiveDownloadIdsAtPreviousShutdown.Contains(t.Id));
                    }
            ...
            {
                Log.Fatal(ex, "Unhandled exception in Client_LoggedIn");
            }
        }
