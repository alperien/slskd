        ...
        if (Subscriptions.TryGetValue(typeof(T), out var subscribers))
        {
            ...
            _ = Task.WhenAll(subscribers.Select(subscriber =>
                Task.Run(async () =>
                {
                    try
                    {
                        await (subscriber.Value as Func<T, Task>)(data);
                    }
                    catch (Exception ex)
                    {
                        Log.Error(ex, "Subscriber {Name} for {Type} encountered an error: {Message}", subscriber.Key, typeof(T), ex.Message);
                    }
                })));
        }
        else
        ...
        if (Subscriptions.TryGetValue(typeof(Event), out subscribers))
        {
            ...
            _ = Task.WhenAll(subscribers.Select(subscriber =>
                Task.Run(async () =>
                {
                    try
                    {
                        await (subscriber.Value as Func<Event, Task>)(data);
                    }
                    catch (Exception ex)
                    {
                        Log.Error(ex, "Catch-All Subscriber {Name} for {Type} encountered an error: {Message}", subscriber.Key, typeof(Event), ex.Message);
                    }
                })));
        }
    }
