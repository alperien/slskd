    ...
    private async Task HandleEvent(Event data)
    {
        ...
        foreach (var webhook in webhooksTriggeredByThisEventType)
        {
            _ = Task.Run(async () =>
            {
                try
                {
                    ...
                    try
                    {
                        ...
                        await Retry.Do(
                            ...
                            isRetryable: (attempts, ex) => ex is HttpRequestException, // retry transient HTTP failures only
                ...
                {
                    Log.Warning(ex, "Unhandled exception calling webhook '{Name}': {Message}", webhook.Key, ex.Message);
                }
            });
        }
    }
