...
namespace slskd.Transfers.Downloads
{
    ...
    public class DownloadService : IDownloadService
    {
        ...
        public bool TryFail(Guid id, Exception exception)
        {
            ...
            // Retry.Do always wraps the final exception in AggregateException; unwrap to preserve the real error.
            if (exception is AggregateException aex && aex.InnerExceptions.Count > 0)
            {
                exception = aex.InnerExceptions[^1];
            }
        ...
        {
            // Unwrap AggregateException in case callers didn't (e.g. Retry.Do wrapper).
            if (exception is AggregateException aex && aex.InnerExceptions.Count > 0)
            {
                exception = aex.InnerExceptions[^1];
            }
            ...
            if (!OptionsMonitor.CurrentValue.Transfers.Download.AutoReplace.Enabled)
            {
                return;
            }
        ...
        private async Task<Transfer> DownloadAsync(Transfer transfer, Action<Transfer> stateChanged = null, CancellationToken cancellationToken = default)
        {
            ...
            {
                // Retry.Do always wraps in AggregateException; unwrap so logging, TryFail, and
                // TryAutoReplaceOnFailureAsync see the real exception (TimeoutException, etc.).
                var unwrapped = ex is AggregateException aex && aex.InnerExceptions.Count > 0
                    ? aex.InnerExceptions[^1]
                    : ex;

                Log.Error(unwrapped, "Download of {Filename} from user {Username} failed: {Message}", transfer.Filename, transfer.Username, unwrapped.Message);

                TryFail(transfer.Id, exception: unwrapped);

                await TryAutoReplaceOnFailureAsync(transfer, unwrapped);
