...
public class AutoReplaceService : IAutoReplaceService
{
    ...
    private ConcurrentDictionary<Guid, byte> InFlightReplacements { get; } = new();
    private static readonly SemaphoreSlim ScanConcurrency = new(5, 5);
    private static readonly TimeSpan LastAttemptCleanupInterval = TimeSpan.FromMinutes(10);
    private DateTime LastAttemptLastCleanup = DateTime.UtcNow;
    ...
    public static string BuildQuery(string filename)
    {
        ...
        // prefer multi-character terms for a more specific search
        if (terms.Count > 0)
        {
            return string.Join(' ', terms).Trim();
        }

        // if the cleaned name is meaningful enough, use it as-is;
        // otherwise return empty and let the browse fallback handle it
    ...
    public async Task<bool> TryReplaceAsync(Transfer transfer, AutoReplaceReason reason, CancellationToken cancellationToken = default)
    {
        ...
        // prevent duplicate concurrent replacements for the same transfer
        // (e.g. inline + background scan racing on the same failure)
        if (!InFlightReplacements.TryAdd(transfer.Id, 0))
        {
            Log.Debug("Auto-replace for {Filename} is already in progress; skipping duplicate", transfer.Filename);
            return false;
        }
        ...
        try
        {
            ...
            // periodically trim the LastAttempt dictionary to prevent unbounded growth
            if (LastAttempt.Count > 1000 && (DateTime.UtcNow - LastAttemptLastCleanup) > LastAttemptCleanupInterval)
            {
                var cutoff = DateTime.UtcNow.AddHours(-1);
                foreach (var kvp in LastAttempt.Where(kvp => kvp.Value < cutoff).ToList())
                {
                    LastAttempt.TryRemove(kvp.Key, out _);
                }
                LastAttemptLastCleanup = DateTime.UtcNow;
            }
            ...
            // Strategy 2: relaxed matching — same search results but drop strict filters,
            //             while still requiring a reasonable size match
            if (candidate is null)
            {
                var relaxed = new slskd.Options.TransfersOptions.GlobalDownloadOptions.AutoReplaceOptions.MatchOptions
                {
                    RequireFreeUploadSlot = false,
                    RequireSameExtension = false,
                    RequireExactSize = true,
                    SizeToleranceBytes = Math.Max(options.Match.SizeToleranceBytes, 10 * 1024 * 1024),
                    MinimumUploadSpeed = 0,
                };

                candidate = AutoReplaceMatcher.SelectBest(
                    originalFilename: transfer.Filename,
                    originalSize: transfer.Size,
                    responses: responses,
                    excludedUsernames: attempted,
                ...
                if (candidate is null)
                {
                    Log.Debug("Auto-replace exact match for {Filename} found no candidate; relaxed match also found none", transfer.Filename);
                }
                else
                {
                    Log.Information("Auto-replace found alternate source via relaxed matching for {Filename} from {Username}", transfer.Filename, candidate.Username);
                }
            }

            // Strategy 3: browse fallback — search the original user's shares directly
            if (candidate is null && !string.IsNullOrWhiteSpace(transfer.Username))
            {
                var browseResponses = await BrowseAsync(transfer.Username, options, cancellationToken);

                // don't exclude the original user for browse; we're deliberately checking their shares
                var browseExcluded = new HashSet<string>(attempted, StringComparer.OrdinalIgnoreCase);
                browseExcluded.Remove(transfer.Username);

                candidate = AutoReplaceMatcher.SelectBest(
                    originalFilename: transfer.Filename,
                    originalSize: transfer.Size,
                    responses: browseResponses,
                    excludedUsernames: browseExcluded,
                    options: options.Match);

                if (candidate is not null)
                {
                    Log.Information("Auto-replace found alternate source via browse fallback for {Filename} from {Username}", transfer.Filename, candidate.Username);
                }
            }

            if (candidate is null)
            {
                // persist the failed attempt so ReplacementAttempts/MaxCandidates
                // eventually gives up even when no alternate source exists
                transfer.ReplacementAttempts++;
                transfer.AttemptedUsernames = FormatUsernames(attempted);

                try
                {
                    Downloads.Update(transfer);
                }
                catch (Exception ex)
                {
                    Log.Debug(ex, "Failed to persist auto-replace lineage for {Filename}", transfer.Filename);
                }
        ...
        {
            InFlightReplacements.TryRemove(transfer.Id, out _);
        }
    }
    ...
    private async Task ScanForStallsAsync(slskd.Options.TransfersOptions.GlobalDownloadOptions.AutoReplaceOptions options, CancellationToken cancellationToken)
    {
        ...
        var tasks = stalled
            ...
            {
                await ScanConcurrency.WaitAsync(cancellationToken);
                try
                {
                    Log.Information("Auto-replace detected a stalled download: {Filename} from {Username}", transfer.Filename, transfer.Username);
                    return await TryReplaceAsync(transfer, AutoReplaceReason.Stall, cancellationToken);
                }
                finally
                {
    ...
    private async Task ScanForFailuresAsync(slskd.Options.TransfersOptions.GlobalDownloadOptions.AutoReplaceOptions options, CancellationToken cancellationToken)
    {
        ...
        var tasks = failed
            .Where(t => !alreadyReplaced.Contains(t.Id))
            .Select(async transfer =>
            {
                await ScanConcurrency.WaitAsync(cancellationToken);
                try
                {
                    return await TryReplaceAsync(transfer, AutoReplaceReason.Failure, cancellationToken);
                }
                finally
                {
                    ScanConcurrency.Release();
    ...
    private async Task<IEnumerable<Response>> BrowseAsync(string username, slskd.Options.TransfersOptions.GlobalDownloadOptions.AutoReplaceOptions options, CancellationToken cancellationToken)
    {
        try
        {
            ...
            var allFiles = browseResponse.Directories
                ...
                .Select(f => new slskd.Search.File
                {
                    ...
                    Filename = f.Filename,
                    IsVariableBitRate = f.IsVariableBitRate,
                    Length = f.Length,
            ...
            return new[]
            {
                new Response
                {
                    ...
                    UploadSpeed = Math.Max(options.Match.MinimumUploadSpeed, 1),
                    QueueLength = 0,
                },
            };
        }
        ...
        {
            Log.Debug(ex, "Auto-replace browse of {Username} failed: {Message}", username, ex.Message);
            return Enumerable.Empty<Response>();
        }
    }
