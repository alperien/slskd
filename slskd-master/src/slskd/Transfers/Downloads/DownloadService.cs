...
namespace slskd.Transfers.Downloads
{
    ...
    public interface IDownloadService
    {
        ...
        /// <param name="metadata">Optional audio metadata keyed by filename, populated from search/browse responses.</param>
        ...
        Task<(List<Transfer> Enqueued, List<(string Filename, string Message)> Failed)> EnqueueAsync(string username, IEnumerable<(string Filename, long Size)> files, Guid? batchId = null, TransferLineage lineage = null, IReadOnlyDictionary<string, TransferFileMetadata> metadata = null, CancellationToken cancellationToken = default);
    ...
    public class DownloadService : IDownloadService
    {
        ...
        /// <exception cref="ArgumentException">Thrown when no files are requested.</exception>
        /// <exception cref="AggregateException">Thrown when at least one of the requested files throws.</exception>
        public async Task<(List<Transfer> Enqueued, List<(string Filename, string Message)> Failed)> EnqueueAsync(string username, IEnumerable<(string Filename, long Size)> files, Guid? batchId = null, TransferLineage lineage = null, IReadOnlyDictionary<string, TransferFileMetadata> metadata = null, CancellationToken cancellationToken = default)
        {
                ...
                */
                foreach (var file in fileList)
                {
                    ...
                    try
                    {
                        ...
                        */
                        var fileMeta = metadata is not null && metadata.TryGetValue(file.Filename, out var m) ? m : null;
                        ...
                        var transfer = new Transfer()
                        {
                            ...
                            IsVariableBitRate = fileMeta?.IsVariableBitRate,
