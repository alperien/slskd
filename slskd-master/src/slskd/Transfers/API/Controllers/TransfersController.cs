    ...
    public class TransfersController : ControllerBase
    {
        ...
        public async Task<IActionResult> EnqueueAsync([FromRoute, UrlEncoded, Required] string username, [FromBody] IEnumerable<QueueDownloadRequest> requests)
        {
            ...
            try
            {
                ...
                var metadata = requests
                    .Where(r => r.BitRate.HasValue || r.BitDepth.HasValue || r.Length.HasValue || r.SampleRate.HasValue || r.IsVariableBitRate.HasValue)
                    .ToDictionary(
                        keySelector: r => r.Filename,
                        elementSelector: r => new TransferFileMetadata
                        {
                            BitRate = r.BitRate,
                            BitDepth = r.BitDepth,
                            Length = r.Length,
                            SampleRate = r.SampleRate,
                            IsVariableBitRate = r.IsVariableBitRate,
                        });

                var (enqueued, failed) = await Transfers.Downloads.EnqueueAsync(username, requests.Select(r => (r.Filename, r.Size)), metadata: metadata);
